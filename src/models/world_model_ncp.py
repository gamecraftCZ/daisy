...
class NcpConfig:
    ncp_units: int
    max_blocks: int  # TODO, wtf is this?


class WorldModelNcp(nn.Module):
    def __init__(self, obs_vocab_size: int, act_vocab_size: int, config: NcpConfig) -> None:
        super().__init__()
        ...
        #     max_blocks=config.max_blocks,  # WTF, how this works?
        ...
        #     CfC() for _ in range(config.num_layers - 1)
        # ])

        self.ncp_layer_wiring = AutoNCP(config.ncp_units, obs_vocab_size)
        self.ncp_layer = CfC(obs_vocab_size, self.ncp_layer_wiring)
